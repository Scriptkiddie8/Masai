...
class Board {
  ...
  private grid: Cell[][];
  private centerLock: string | null = null;

  constructor() {
    this.grid = Array.from({ length: this.size }, () =>
      Array.from({ length: this.size }, () => "_" as Cell)
    );
  }
  ...
  private updateDiagonalLock(symbol: string): void {
  ...
  placeOrThrow(r1: number, c1: number, symbol: string): void {
    const r = r1 - 1,
      c = c1 - 1;
  ...
  hasWin(symbol: string): boolean {
    for (let i = 0; i < 3; i++) {
      if (
        this.grid[i][0] === symbol &&
        this.grid[i][1] === symbol &&
        this.grid[i][2] === symbol
      )
        return true;
      if (
        this.grid[0][i] === symbol &&
...
class Game {
  ...
  private async registerPlayers(): Promise<void> {
    ...
    if (!name1 || !name2) throw new Error("Names cannot be empty.");
    if (name1 === name2) throw new Error("Player names must be unique.");

    const sym1 = (
      await this.ask(`${name1} symbol (single char, not '_'): `)
    ).trim();
    const sym2 = (
      await this.ask(`${name2} symbol (single char, not '_', not ${sym1}): `)
    ).trim();

    const validSym = (s: string) =>
      s.length === 1 && s !== "_" && !/\s/.test(s);
    if (!validSym(sym1) || !validSym(sym2))
      throw new Error("Invalid symbol(s).");
    if (sym1 === sym2) throw new Error("Symbols must be unique.");
  ...
  private async promptMove(): Promise<boolean> {
    const p = this.current();
    const input = (
      await this.ask(`${p.name} (${p.symbol}) move [row col] or 'q' to quit: `)
    ).trim();
